(function() {
  ...
  if (numbering > 0)
    CSSRules.push(
      '#h5o-inside li::before{\
		content:' +
        (numbering === 2 ? 'counters(li,".")' : 'counter(li)') +
        '".";\
		display:block;\
		left:-10.5em;\
		position:absolute;\
  ...
  function printSection(section) {
    ...
    if (showDetails) {
      ...
      if (section.explicit)
        details +=
          '<' + section.associatedNodes[0].nodeName.toLowerCase() + '>';
  ...
  function HTMLOutline(root) {
    ...
    function enter(node) {
      if (isElement(node)) {
        if (
          !stack.isEmpty() &&
          (isHeadingContentElement(stack.top) || isHidden(stack.top))
        ) {
          // Do nothing
          ...
          if (currentOutlinee !== null) {
            if (hasNoHeading(currentSection))
              createImpliedHeading(currentSection);
          ...
          if (hasNoHeading(currentSection)) currentSection.heading = node;
            ...
            hasImpliedHeading(currentOutlinee.lastSection) ||
            node.rank >= currentOutlinee.lastSection.heading.rank
          ) {
            ...
            while (node.rank >= candidateSection.heading.rank)
    ...
    function exit(node) {
      if (isElement(node)) {
        if (!stack.isEmpty() && node === stack.top) stack.pop();
          ...
          isSectioningContentElement(node) ||
          isSectioningRootElement(node)
        ) {
          if (hasNoHeading(currentSection))
            createImpliedHeading(currentSection);
    ...
    function isSectioningContentElement(node) {
      return (
        ['article', 'aside', 'nav', 'section'].indexOf(
